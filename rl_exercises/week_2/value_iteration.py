...
def value_iteration(
    *,
    ...
    Q = R_sa + gamma * np.einsum("san,n->sa", T, V)
    best_actions = Q == np.max(Q, axis=1, keepdims=True)
    pi = np.array([rng.choice(np.flatnonzero(actions)) for actions in best_actions])
